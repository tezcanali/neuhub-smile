...
<div class="hero-page w-100 h-250px overflow-hidden position-relative">
    <div class="container-xxl position-relative z-3 h-100 py-4 d-flex justify-content-center flex-column">
        ...
        <div class="d-flex align-items-center gap-3 text-primary">
            ...
            <div class="fs-54 fw-bold text-primary">{{ $page->title}}</div>
        </div>
    </div>
...
@php
    ...
    $categories = \App\Models\GalleryCategory::where('type',2)->get();
@endphp
...
<div class="page py-lg-5 py-4">
    <div class="container-xxl">
        <div class="bg-white rounded-3 p-2 d-flex flex-wrap align-items-center gap-2 mb-4">
            <a href="#" class="fw-normal bg-secondary text-white rounded-1 py-2 px-4">All</a>
            @foreach($categories as $category)
                <a href="/before-after/category/{{$category->slug}}" class="fw-normal bg-gray-100 rounded-1 py-2 px-4">{{ $category->title }}</a>
            @endforeach
        </div>
        <div class="row g-4">
